...
def problem2b(rect, n, delta, win):
    ...
    win.render()
    for k in range(n):
        rect1 = rg.Rectangle(rg.Point(rect.get_lower_left_corner().x - (k * delta), rect.get_lower_left_corner().y - (k * delta)), rg.Point(rect.get_upper_right_corner().x + (k * delta), rect.get_upper_right_corner().y + (k * delta)))
        rect1.attach_to(win)
        win.render()
